...
Consumes quarantined poison pills from Redpanda 'dead.letter.queue' or S3 quarantine,
...
class DLQTriageEngine:
    ...
    def classify_error(self, raw_record: str | bytes | dict[str, Any]) -> dict[str, Any]:
        ...
        # Identify schema to test against
        target_schema = None
        if "order_id" in inner_payload or "total_amount_baht" in inner_payload:
            target_schema = (
                self.schemas.get("orders.lifecycle.v1.json")
                or self.schemas.get("OrderLifecycleEvent")
                or self.schemas.get("https://platform.internal/schemas/orders/orders.lifecycle.v1.json")
            )
        elif "rider_id" in inner_payload:
            target_schema = (
                self.schemas.get("riders.telemetry.v1.json")
                or self.schemas.get("RiderTelemetryEvent")
                or self.schemas.get("https://platform.internal/schemas/riders/riders.telemetry.v1.json")
            )
